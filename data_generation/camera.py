...
class Camera(object):
    ...
    def __init__(self, env, near=0.1, far=100.0, image_size1=448, image_size2=448, dist=5.5, \
        ...
        self.camera = env.scene.add_mounted_camera(
            ...
            pose=Pose(),  # relative to the mounted actor
            width=image_size1,
            height=image_size2,
            fovy=np.deg2rad(35),
            near=near,
        ...
        self.phi = phi
        self.pos = pos
        self.width=image_size1
        self.height=image_size2
    ...
    def get_observation(self):
        ...
        depth = self.camera.get_float_texture('Position').astype(np.float32)[:,:,3]
        return rgb, depth
    ...
    def compute_camera_XYZA(self, depth):
        camera_matrix = self.camera.get_intrinsic_matrix()[:3, :3]
    ...
    def get_movable_link_mask(self, link_ids):
        link_seg = self.camera.get_uint32_texture('Segmentation')
        # print('max_0: ', np.max(link_seg[:,:,0]), np.where(link_seg[:, :, 0]==4))
        # print('max_1: ', np.max(link_seg[:,:,1]), np.where(link_seg[:, :, 1]==4))
        # assert np.max(np.abs(link_seg[:,:,1]-link_seg[:,:,0]))==0
        ## We cannot decide whether to use link_seg[0] or link_seg[1] as the segmentation mask,
        ## so we suppose that they are the same.
        link_seg=link_seg[:, :, 1]
        link_mask = np.zeros((link_seg.shape[0], link_seg.shape[1])).astype(np.uint8)
        for idx, lid in enumerate(link_ids):
            # print('lid: ', lid, int(lid))
    ...
    def get_metadata(self):
        return {
            ...
            'height': self.height,
            'fov': self.fovy,
            'camera_matrix': self.camera.get_intrinsic_matrix(),
    ...
    def get_metadata_json(self):
        return {
            ...
            'camera_matrix': self.camera.get_intrinsic_matrix().tolist(),
