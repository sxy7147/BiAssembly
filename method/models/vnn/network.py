...
''' 
 Note that the GT pc is a whole pc and can not be split to two parts(?)
    known the transformation from init pc to GT pc (?)
    disassembly direction (only translation?) : inv to init pc geometry & GT pc geometry, equ to GT pc rotation (object's attribute) (cVAE?) 
    target pose (rotation + translation on GT pc): related to int pc (relative pose) & disassembly direction & direction (cVAE?)
        input: feature of concat(init pc geometry, GT pc geometry, equ GT pc rotation) + init part position(pc center) + disassembly direction + robot position
    Critic: select a combination of disassembly direction & target pose
    
    pick-up cVAE
    Critic: selec a action on init parts + reorient parts + assembled parts
    
    In data collection, add new data to be saved: init parts' position, point cloud of each part in init state and assembled state (the latter one can be acquired directly from mesh.obj)
'''
...
class VNNModel(BaseModel):
    def __init__(self, cfg):
        ...
        self.cfg.model.encoder == "vn_dgcnn"
        # self.cfg.model.regressor == "original"
        self.cfg.model.regressor == "original_cvae"
        
        self.encoder = self.init_encoder()
        self.pose_predictor = self.init_pose_predictor()
        # self.cvae_encoder = cVAEEncoder(input_dim=self.cfg.model.pc_feat_dim * 2, output_dim=32)
    ...
    def init_encoder(self):
        if self.cfg.model.encoder == "dgcnn":
            ...
            encoder = VN_DGCNN(feat_dim=self.cfg.model.pc_feat_dim)     # the vector neuron version of DGCNN
        return encoder
    ...
    def init_pose_predictor(self):
        ...
        elif self.cfg.model.regressor == "original_cvae":
            pose_predictor = cVAE(input_dim=self.cfg.model.pc_feat_dim * 2, output_dim=32)
    ...
    def _extract_part_feats(self, part_pcs, part_valids):
        ...
        valid_feats_equiv, valid_feats_inv = self.encoder(part_pcs)
    ...
    def forward(self, gt_pcs, disassembly_dir):    # 后面gt pcs & init pcs最好是单独的？起码gt pcs应该可以是单独的，然后features应该也是B * P * N * 3
        # GT_pcs = GT_pcs.permute(0, 1, 3, 2)
        # gt_pcs = gt_pcs.permute(0, 2, 1)
        B, P, _, _ = gt_pcs.shape  # [B, P, N, 3]
        
        gt_pcs = gt_pcs.permute(0, 1, 3, 2)
        print('GT_pcs: ', gt_pcs.shape)
        part_valids = torch.ones(gt_pcs.shape[0], gt_pcs.shape[1]).to(gt_pcs.device)    # TODO: we can remove the part_valids, here we simply follow the previous code base.
        ...
        print('gt_equiv_feats: ', gt_equiv_feats.shape)
        print('gt_inv_feats: ', gt_inv_feats.shape)
        ...
        print('global_equ_feats: ', global_equ_feats.shape)
        print('global_inv_feats: ', global_inv_feats.shape)
        
        losses = self.pose_predictor.get_loss(global_equ_feats, global_inv_feats, disassembly_dir)
        
        return losses
        
        
        
        # init_pcs = init_pcs.permute(0, 2, 1)
        # print('init_pcs: ', init_pcs.shape)
        # init_equiv_feats, init_inv_feats = self._extract_part_feats(init_pcs)  # (batch_size, num_parts, 2*feat_dim, 3), (batch_size, num_parts, 2*feat_dim, 2*feat_dim)
        
        # gt_equiv_feats * gt_inv_feats * init_inv_feats
        # GF = torch.bmm(init_inv_feats, gt_inv_feats)
        # print('GF1: ', GF.shape)
        # GF = torch.bmm(GF, gt_equiv_feats)
        # print('GF2: ', GF.shape)
        
        
        
        # get the mu and sigma of cVAE
        ...
        # part_valids = batch_data["part_valids"] # part_valids`: binary mask indicating padded parts.
    ...
    def _loss_function(self, data_dict, out_dict={}, optimizer_idx=-1, mode="train"):   # main function
